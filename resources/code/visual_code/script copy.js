...
// canvasHeight = canvasWidth/1.356
...
const forward = () => {
  ...
  let angle = 0 - (playerObject.rotation * 180) / Math.PI;
...
const left = () => {
  ...
  playerObject.rotation = (-angle * Math.PI) / 180;
};
...
const matrix = (level) => {
  ...
  tileXOffset = (screenWidth - totalTilesWidth) / 2;
  tileYOffset = (screenHeight - totalTilesHeight) / 2;
  ...
  // creating the player sprite object. it is generated after making the tiles, so that its rendered on top
  playerObject = new PIXI.Sprite(spriteBear);
  ...
  PIXI.Loader.shared
    ...
    .load((loader, resources) => {
      ...
      tileGray = new PIXI.Texture(
        tileSheet,
        new PIXI.Rectangle(
          tilesData.frames.tileGray.frame.x,
          tilesData.frames.tileGray.frame.y,
          tilesData.frames.tileGray.frame.w,
          tilesData.frames.tileGray.frame.h
        )
      );
      ...
      spriteBear = new PIXI.Texture(
        spriteDataSheet,
        new PIXI.Rectangle(
          spriteData.frames.bear.frame.x,
          spriteData.frames.bear.frame.y,
          spriteData.frames.bear.frame.w,
          spriteData.frames.bear.frame.h
        )
      );
